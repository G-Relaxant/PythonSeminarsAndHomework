...
# пример решения преподавателя, вместо списков используются словари, кода гораздо меньше и проще

dict = {"АВЕИНОРСТ":1, "ДКЛМПУ":2 }
...
for key, val in dict.item:
    if symbol.upper() in key:
        sum += val
# это не вся задача, а только поиск значений для букв
