"""
Дан список, состоящий из целых чисел. Напишите программу, которая сформирует список из тех элементов, которые больше предыдущего (элемента с предыдущим номером)

Примечание: Пользователь может вводить значения списка или список задан изначально.

Примеры/Тесты:

Input: [0, -1, 5, 2, 1]

Output: [5]

Input: [-2, -1, 5, 2, 3]

Output: [-1, 5, 3]

[*] Усложнение: Запишите алгоритм в одну строку, используйте Comprehension
"""



list_1 = [0, -1, 5, 2, 3]
result = []
count = 0

# for i in range(1, len(list_1)):         # вариант цикла в классическом виде
#     if list_1[i - 1] < list_1[i]:
#         result.append(list_1[i])
# print(*result)


result2 = [list_1[i] for i in range(1, len(list_1)) if list_1[i - 1] < list_1[i]]    # вариант цикла в виде Comprehension
print(result2)
